...
let totalResult = document.getElementsByClassName("price-container");

Array.from(totalResult).forEach(val => {
...
const totalPriceFunc = () => {
  ...
  if (
    ...
    product.value !== "" &&
    isNaN(productPrice.value) === false &&
    isNaN(numberPurchased.value) === false &&
    isNaN(amountPaid.value) === false
  ) {
    let generate = confirm(
      "If you proceed, the receipt would be generated. CLICK OK TO CONTINUE or CANCEL TO RETURN"
    );
    if (generate) {
    ...
    if (newAmountPaid === "" || newAmountPaid === NaN) {
      ...
      document.getElementsByTagName("input")[5].placeholder =
        "Input a Number of item";
...
const toPrint = () => {
  ...
  a.document.write(
    " * {box-sizing: border-box;} table{font-family: serif; font-weight: 100; width: 150px; max-width: auto; margin: auto;}  caption {font-weight: 900; margin-left: 105%; font-size: 15px;}  table,th, td {border: 0.2px solid black; border-collapse: collapse; border: 0.2px solid black; padding: 5px; font-size: xx-small;} th,td {text-align: center;}"
  );
